...
function makeFeatures(phnFname, wavFname)
    ...
    feats = mfcc(samps, sr, minfreq=64.0, maxfreq=8000.0)[1]
    ...
    feats = feats[[i for i in 1:size(feats,1) if !(i in Set(idxsToDelete
    ))],:]

    feats_deltas = deltas(feats)
    #feats_delta_deltas = deltas(feats_deltas)
    features = hcat(feats, feats_deltas)
    #features = hcat(feats, feats_deltas, feats_delta_deltas)
...
function createData(data_dir, out_dir)
    for (root, dirs, files) in walkdir(data_dir)
        ...
        phnFnames = [x for x in files if occursin("PHN", x) && ! occursin("BUCK", x)]
